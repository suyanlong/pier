...
func (e *ChannelExecutor) Rollback(ibtp *pb.IBTP, isSrcChain bool) {
	if err := retry.Retry(func(attempt uint) error {
		...
		if err != nil {
			e.logger.Errorf("Execute rollback tx: %s, retry sending tx", err.Error())
			return fmt.Errorf("execute rollback tx: %w", err)
		}

		if !resp.Status {
			e.logger.Errorf("Execute rollback tx got failed response: %s, retry sending tx", resp.Message)
			return fmt.Errorf("execute rollback tx: %w", err)
		}
